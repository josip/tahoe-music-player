...
var SingleThreadedMapReducer = {
  /**
   *  SingleThreadedMapReducer.map(map, dict, progress, chunkSize, finished) -> undefined
  ...
  /**
   *  SingleThreadedMapReducer.reduce(reduce, mapResult, progress, chunkSize, finished) -> undefined
   *  - reduce (Function): reduce function.
   *  - mapResult (Object): Object returned by [[SingleThreadedMapReducer.map]].
  ...
  reduce: function STMR_reduce(reduce, mapResult, progress,
    ...
    function continueReduce() {
      ...
      do {
        ...
        // TODO: This looks like a bug from original implementation?
        // since `keys` are initialized a line before its length will
        // be always 0, thus making this unnecessary?
...
function DB(name, storage, dict, cb) {
  var self = this;
  ...
  function commitToStorage(cb) {
    if (!cb)
      cb = function() {};
    storage.put(dbName, dict.dict, cb);
  }
  ...
  this.put = function DB_put(document, cb) {
    if (isArray(document)) {
      dict.setDocs(document);

      // var n = document.length, doc;
      //       while(n--) {
      //         doc = document[n];
      //         dict.set(doc.id, doc);
      //         
      ...
      function(mapResult) {
        if (options.reduce)
          mapReducer.reduce(
            ...
            chunkSize,
            function(rows) {
              options.finished(new View(rows));
            });
        else
          options.finished(new MapView(mapResult));
      });
  };
...
function MapView(mapResult) {
  ...
  this.include = function (mapResult) {
    ...
    for(var i = 0, ii = mapKeys.length; i < ii; i++) {
      ...
      this.rows = this.rows.concat(newRows.sort(idSort));
    }
  };
...
var BrowserCouch = {
  get: function BC_get(name, cb, storage) {
    ...
    new DB(name, storage, new Dictionary(), cb);
  },
...
/*
  ...
  _DB: function BC__DB(name, storage, dict, cb) {
    ...
    this.put = function DB_put(document, cb) {
      if (isArray(document)) {
        var n = document.length;
        while(n--)
          dict.set(document[n].id, document[n]);
      } else
        dict.set(document.id, document);
  ...
  _MapView: function BC__MapView(mapResult) {
    ...
    for (var i = 0, ii = mapKeys.length; i < ii; i++) {
      ...
      var newRows = [];
      for (var j = 0, jj = item.keys.length; j < jj; j++) {
        var id = item.keys[j];
